  ...
  describe('overwriteFileAsync', function () {
    it('overwrites the binary content', async function () {
      ...
      const doc: Metadata = {
        ...
        lastModification: '2015-11-16T16:12:01.002Z', // FIXME
        sides: {
          local: 1
        }
      }
      ...
      file.should.have.properties({
        ...
        name: created.name
        // FIXME: cozy.files.updateFileById(..., {...lastModifiedDate...}) ?
    ...
    it('throws an error if the checksum is invalid', async function () {
      const created = await builders.file().build()
      const old = {
        ...(conversion.createMetadata(created)),
      ...
      this.remote.other = {
        createReadStreamAsync (localDoc) {
          localDoc.should.equal(doc)
          const stream = builders.stream().build()
          return Promise.resolve(stream)
        }
      }
      */

      return should(this.remote.overwriteFileAsync(doc, old))
